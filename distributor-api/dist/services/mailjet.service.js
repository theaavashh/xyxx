...
class MailjetEmailService {
    ...
    initializeMailjet() {
        if (process.env.MAILJET_API_KEY && process.env.MAILJET_SECRET_KEY) {
            this.mailjet = node_mailjet_1.default.apiConnect(process.env.MAILJET_API_KEY, process.env.MAILJET_SECRET_KEY);
            logger_1.default.info('Mailjet service initialized successfully');
        }
        else {
            logger_1.default.warn('Mailjet service not initialized - API credentials missing');
        }
    }
    async sendEmail(notification) {
        if (!this.mailjet) {
            logger_1.default.warn('Email not sent - Mailjet not initialized', {
                to: notification.to,
        ...
        try {
            ...
            logger_1.default.info('Email sent successfully via Mailjet', {
                to: notification.to,
                subject: notification.subject,
            ...
            logger_1.default.error('Failed to send email via Mailjet', error, {
                to: notification.to,
                subject: notification.subject
            });
            return false;
    ...
    distributorApprovedTemplate(data) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    distributorApprovedWithCredentialsTemplate(data) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="info-section">
              ...
              <ul style="margin: 10px 0; padding-left: 20px;">
                ${data.categories.map((category) => `<li>${category.title}</li>`).join('')}
              </ul>
            </div>
            ` : ''}
    ...
    distributorOfferLetterTemplate(data) {
    ...
    async notifyDistributorApproved(applicationData, credentials) {
    ...
    async sendDistributorOfferLetter(applicationData, products, additionalData) {
...
exports.default = mailjetEmailService;
