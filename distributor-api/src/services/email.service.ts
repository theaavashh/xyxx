...
class EmailService {
  ...
  private initializeTransporter(): void {
    ...
    if (process.env.SMTP_HOST && process.env.SMTP_PORT && process.env.SMTP_USER && process.env.SMTP_PASS) {
      ...
      logger.warn('Email service not initialized - SMTP configuration missing');
    }
  }
  ...
  async sendEmail(notification: EmailNotification): Promise<boolean> {
    if (!this.transporter) {
      logger.warn('Email not sent - transporter not initialized', { to: notification.to, subject: notification.subject });
      return false;
    }
  ...
  private applicationSubmittedTemplate(data: any): string {
    return `
      <!DOCTYPE html>
      <html>
      <head>
  ...
  // Application status updated template
  private applicationStatusUpdatedTemplate(data: any): string {
  ...
  private applicationApprovedTemplate(data: any): string {
    return `
      <!DOCTYPE html>
      <html>
      <head>
  ...
  private applicationRejectedTemplate(data: any): string {
    return `
      <!DOCTYPE html>
      <html>
      <head>
  ...
  async notifyApplicationSubmitted(applicationData: any): Promise<void> {
  ...
  async notifyApplicationStatusUpdate(applicationData: any): Promise<void> {
    const notification: EmailNotification = {
      to: applicationData.email,
